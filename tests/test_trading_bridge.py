...
def test_trading_bridge(tmp_path):
    ...
    cfg1 = yaml.safe_load(Path("config/run_fund_selection.yaml").read_text(encoding="utf-8"))
    ...
    cfg2 = yaml.safe_load(Path("config/run_trade_from_allocations.yaml").read_text(encoding="utf-8"))
    cfg2["artifacts"]["root"] = str(tmp_path)
    cfg2["plugins"]["pipeline"]["params"]["allocations_path"] = str(alloc_path)
    cfg2["plugins"]["pipeline"]["params"]["instrument_map"] = "config/instruments.yaml"
