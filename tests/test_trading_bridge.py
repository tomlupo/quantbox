...
def test_trading_bridge(tmp_path):
    ...
    cfg1 = yaml.safe_load(Path("cookbook/configs/run_fund_selection.yaml").read_text(encoding="utf-8"))
    ...
    cfg2 = yaml.safe_load(Path("cookbook/configs/run_trade_from_allocations.yaml").read_text(encoding="utf-8"))
    cfg2["artifacts"]["root"] = str(tmp_path)
    cfg2["plugins"]["pipeline"]["params"]["allocations_path"] = str(alloc_path)
    cfg2["plugins"]["pipeline"]["params"]["instrument_map"] = "cookbook/configs/instruments.yaml"
