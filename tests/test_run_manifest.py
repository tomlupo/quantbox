...
def test_manifest_written(tmp_path):
    ...
    cfg = yaml.safe_load(Path("config/run_fund_selection.yaml").read_text(encoding="utf-8"))
...
def test_manifest_includes_reproducibility_evidence(tmp_path):
    cfg_path = Path("config/run_fund_selection.yaml")
    ...
    assert manifest["config"]["path"].endswith("config/run_fund_selection.yaml")
