...
_VIZ_ERROR = "matplotlib and seaborn are required for visualization. Install with: uv pip install 'quantbox[viz]'"
...
class SimulationPlotter:
    ...
    def plot_simulation_paths(
        ...
        for i, (lower_p, upper_p) in enumerate(
            zip(percentiles[: len(percentiles) // 2], percentiles[len(percentiles) // 2 + 1 :][::-1], strict=False)
        ):
    ...
    def plot_correlation_dynamics(
        ...
        asset_pairs: list[tuple[int, int]] | None = None,
        asset_names: list[str] | None = None,
        ...
        for i, (a1, a2) in enumerate(asset_pairs[:10]):
            ...
            ax.plot(
                t, corr_series, label=f"{asset_names[a1]}-{asset_names[a2]}", color=self.colors[i % len(self.colors)]
            )
        ax.set_xlabel("Time")
    ...
    def plot_stress_test_comparison(
        ...
        for bar, val in zip(bars, values, strict=False):
            ax.text(
                val, bar.get_y() + bar.get_height() / 2, f"{val:.1%}", va="center", ha="left" if val >= 0 else "right"
            )
        plt.tight_layout()
        if save_path:
            fig.savefig(save_path, dpi=150, bbox_inches="tight")
    ...
    def plot_risk_metrics(
        ...
        cvar_values: dict[float, float] | None = None,
    ...
    def create_summary_dashboard(
        ...
        ax2.axvline(
            np.mean(terminal_returns), color="red", linestyle="--", label=f"Mean: {np.mean(terminal_returns):.1%}"
        )
        ...
        if simulation_result.correlation_matrix is not None:
            sns.heatmap(
                simulation_result.correlation_matrix,
                annot=True,
                fmt=".2f",
                cmap="RdBu_r",
                center=0,
