#!/usr/bin/env python3
"""
Carver Trend Following - Hyperliquid Live Trading Script

Runs the Carver strategy on Hyperliquid DEX with:
...
Usage:
    ...
    python run_carver_hyperliquid.py --dry-run
    
    # Live trading
    python run_carver_hyperliquid.py --live
    
    # With custom budget (default: uses full account)
    python run_carver_hyperliquid.py --live --budget 1000
    
    # Use testnet
    python run_carver_hyperliquid.py --dry-run --testnet

Requirements:
    - HYPERLIQUID_WALLET (your main wallet address)
    - HYPERLIQUID_PRIVATE_KEY (API wallet private key from https://app.hyperliquid.xyz/API)
...
from quantbox.plugins.broker.hyperliquid import HyperliquidBroker, RiskConfig, send_telegram
from quantbox.plugins.strategies.carver_trend import CarverTrendStrategy
from quantbox.plugins.datasources.binance_data import BinanceDataFetcher  # Use Binance for data (free)
...
# Universe - majors available on Hyperliquid
# Note: Hyperliquid has different symbols than Binance
UNIVERSE = [
...
RISK_CONFIG = RiskConfig(
    ...
    min_order_notional=10.0,  # Hyperliquid minimum
)
...
def fetch_data(universe: list) -> dict:
    """Fetch market data for strategy (using Binance - free data)."""
...
def main():
    parser = argparse.ArgumentParser(description="Carver Trend - Hyperliquid Live Trading")
    ...
    # Check credentials
    if not os.environ.get('HYPERLIQUID_WALLET') or not os.environ.get('HYPERLIQUID_PRIVATE_KEY'):
        print("ERROR: Hyperliquid credentials not set")
        print()
        print("Required environment variables:")
        print("  HYPERLIQUID_WALLET=0x... (your main wallet address)")
        print("  HYPERLIQUID_PRIVATE_KEY=0x... (API wallet private key)")
        print()
        print("Setup:")
        print("  1. Go to https://app.hyperliquid.xyz")
        print("  2. Connect your wallet and deposit USDC")
        print("  3. Go to API tab and generate an API wallet")
        print("  4. Set the env vars above")
        sys.exit(1)
    ...
    if args.dry_run:
        ...
        logger.info("DRY RUN - Connecting to Hyperliquid...")
        
        try:
            broker = HyperliquidBroker(testnet=args.testnet, risk=RISK_CONFIG)
        ...
        print(f"\nAccount Balance: ${balance['total']:,.2f} USDC")
