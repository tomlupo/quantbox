...
def get_lot_size_and_min_notional(symbol_info: dict | None) -> tuple[float, float, float]:
    """
...
class PriceCache:
    ...
    def get(self, asset: str) -> tuple[float | None, bool]:
    ...
    def set(self, asset: str, price: float | None) -> None:
        """Cache a price."""
...
class BinanceLiveBroker:
    ...
    market_sim: MarketSimConfig | None = None
    ...
    # The venue's valuation basis -- see BrokerPlugin.valuation_basis. Spot
    # balances: the book is worth its cash plus its holdings. Note this is a
    # CLASS attribute and a plain constant on purpose -- unlike `describe()`
    # below, which returns a live account snapshot and costs API calls, a
    # valuation rule must be free to read and must not vary with account state.
    ...
    def __post_init__(self):
        ...
        if api_key and api_secret and Client is not None:
            try:
                ...
                logger.info("Connected to Binance API for price data")
            ...
            if not self.paper_trading:
                raise OSError(f"Live mode requires: {self.api_key_env} / {self.api_secret_env}")
    ...
    def get_portfolio_value(self) -> float:
        ...
        logger.info(
            f"Portfolio value: {total:.2f} {self.stable_coin} (longs: {long_value:.2f}, short P&L: {short_unrealized_pnl:.2f})"
        )
        return max(0, total)
    ...
    def generate_rebalancing(self, target_weights: dict[str, float]) -> pd.DataFrame:
        ...
        adjusted_weights = {asset: weight * self.capital_at_risk for asset, weight in target_weights.items()}
    ...
    def _get_paper_holdings(self) -> dict[str, float]:
        ...
        paper_file = os.path.join(self.trades_dir, self.account_name.lower(), "paper_portfolio.json")
    ...
    def _get_paper_short_entries(self) -> dict[str, dict]:
        ...
        short_file = os.path.join(self.trades_dir, self.account_name.lower(), "short_entries.json")
    ...
    def _get_price_cached(self, asset: str) -> float | None:
    ...
    def _fetch_price_with_retry(self, asset: str) -> float | None:
    ...
    def _get_price_via_btc(self, asset: str) -> float | None:
    ...
    def _generate_orders_from_rebalancing(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        total_buy_value = sum(abs(row["Delta_Qty"]) * row["Price"] for _, row in buy_rows.iterrows() if row["Price"])
        ...
        if not orders_df.empty:
            orders_df["Executable"] = (orders_df["Adjusted_Qty"] > 0) & (orders_df["Order_Status"] == "To_be_placed")
        return orders_df

    def _prepare_order(self, row: pd.Series, is_buy: bool, scaling_factor: float = 1.0) -> dict[str, Any]:
        ...
        zero_target_sell = not is_buy and row.get("Target_Weight", 0) == 0 and row.get("Current_Qty", 0) > 0
    ...
    def _execute_single_order(
        ...
        try:
            ...
            if price is None or price == 0:
                res = self._client.create_order(symbol=symbol, side=side, type="MARKET", quantity=qty_str)
            else:
                res = self._client.create_order(
                    symbol=symbol, side=side, type="LIMIT", timeInForce="GTC", quantity=qty_str, price=str(price)
                )
    ...
    def _execute_paper_order(self, symbol: str, side: str, qty: float, price: float | None) -> dict[str, Any]:
        """
    ...
    def _save_trade_history(self, report: dict[str, Any]) -> str | None:
...
filtered_weights = selector.apply_mask(strategy_weights, universe, normalize=True)
    ...
    def get_universe_stats(
        ...
        return {
            ...
            "universe_market_caps": {t: latest_mc.get(t, 0) for t in current_universe},
            "universe_prices": {t: latest_prices.get(t, 0) for t in current_universe},
