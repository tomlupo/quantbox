...
| **Config loading** | `load_yaml()` on `cookbook/configs/accounts/{name}.yaml` + per-strategy `cookbook/configs/strategies/{name}.yaml` | Single YAML config; profiles resolved via bundled `plugins/manifest.yaml` (or `QUANTBOX_MANIFEST` override) |
