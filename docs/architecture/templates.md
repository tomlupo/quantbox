...
- Project structure (`src/`, `configs/`, `research/`, `data/`, `output/`).
...
8. Run the example pipeline as a smoke test (`quantbox run -c configs/example.yaml`).
...
├── pipelines/
│   └── example_backtest.py                 # the script that runs configs/example_backtest.yaml
