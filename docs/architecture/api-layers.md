...
| **L4** Full pipeline | `quantbox.run_from_config(yaml_path)` | Logged experiment, ArtifactStore manifest, EXPERIMENTS.md entry. | `run_from_config("configs/research.yaml")` |
...
result = run_from_config("configs/research/regime_taa.yaml")
```
...
```bash
quantbox run -c configs/prod/dm_evo_fund_selection.yaml --strict
```
